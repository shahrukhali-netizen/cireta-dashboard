...
export default function Icon({ name, className = "w-5 h-5" }) {
  if (FILLED[name]) {
    return <svg className={className} fill="currentColor" viewBox="0 0 24 24">{FILLED[name]}</svg>;
  }
  const inner = ICONS[name];
  if (!inner) return null;
  return <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">{inner}</svg>;
}
